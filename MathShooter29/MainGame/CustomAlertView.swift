...
struct CustomAlertView: View {
    ...
    @State private var showingVolumeEffectLabel = false
    var body: some View {
            ...
            ZStack{
                ...
                VStack {
                    ...
                    if showingVolumeLabel {
                        Text("Music Volume: \(Int(game_Data.volume * 50))%")
                            .font(.system(size: sizeIn.size.width/25, weight: .regular))
                    ...
                    if showingVolumeEffectLabel {
                        Text("Effect Volume: \(Int(game_Data.volumeEffect * 50))%")
                            .font(.system(size: sizeIn.size.width/25, weight: .regular))
                            //.font(.system(size: 30, weight: .regular))
                            .foregroundColor(.black)
                            .padding()
                            .opacity(showingVolumeEffectLabel ? 1 : 0)
                            .animation(.easeInOut(duration: 0.5), value: showingVolumeEffectLabel)
                    }
                    ...
                    HStack{
                        Text("Effect")
                            ...
                            .foregroundColor(.black)
                        Button(action: {
                            let newVolume = max(game_Data.volumeEffect - 0.01, 0)
                            game_Data.volumeEffect = newVolume
                            showLabelOnVolumeEffect()
                            //AudioManager.shared.adjustVolume(volume: newVolume)
                        }) {
                            Image(systemName: "speaker.minus")
                                .resizable()
                        ...
                        Slider(value: Binding(
                            get: {
                                game_Data.volumeEffect
                            },
                            set: { newValue in
                                game_Data.volumeEffect = newValue
                                //AudioManager.shared.adjustVolume(volume: newValue)
                                showLabelOnVolumeEffect()
                            }
                        ), in: 0...2, step: 0.01)
    ...
    func showLabelOnVolumeEffect(){
        withAnimation{
            showingVolumeEffectLabel = true
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            withAnimation{
                showingVolumeEffectLabel = false
            }
        }
    }
}
