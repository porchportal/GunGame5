...
class Game_Scene: SKScene, SKPhysicsContactDelegate{
    ...
    weak var BossTwoFireTimer : Timer?
    var bossTwoLives = 30
    ...
    var ValueOfHard = 0
    var numUnknow1: Double = 0.0
    var numUnknow2: Double = 0.0
    var countmusic = 0
    ...
    init(gameData: gameData ,size: CGSize) {
        ...
        /*
        let backgroundColorNode = SKSpriteNode(color: .blue, size: gameArea.size)
        backgroundColorNode.position = CGPoint(x: gameArea.midX, y: gameArea.midY)
    ...
    override func didMove(to view: SKView){
        ...
        scaleMode = .aspectFit
        //scene?.size = gameArea.size
        let texturesToPreload = [SKTexture(imageNamed: "Number2.001"), SKTexture(imageNamed: "Number4.001"), SKTexture(imageNamed: "32.001"), SKTexture(imageNamed: "buttel.001")]
        SKTexture.preload(texturesToPreload) {
            print("Textures preloaded!")
        }
        sounds.preload("LaserGun")
        sounds.preload("Rocketlauch")
        sounds.preload("HitSound")
    ...
    func startGame(){
        ...
        if let particles = SKEffectNode(fileNamed: "Space"){//CGSize(width: 750, height: 1335)
            //particles.position = CGPoint(x: UIScreen.main.bounds.width, y: UIScreen.main.bounds.height*1.7)
            particles.position = CGPoint(x: frame.width/2, y: 1335)
    ...
    func startNewLevel(){
        ...
        let spwn = SKAction.run{ [weak self] in
            ...
            if self.BossOneActive == false{
                ...
                self.spawnEnemy()
            } else if levelDuration == 0.3 && countNotOverlap == 0{
                self.spawnEnemy()
                self.spawnEnemy()
                levelNumber = 1
                countNotOverlap = 1
            } else if levelDuration == 0.3 && countNotOverlap == 1{
                self.spawnEnemy()
                self.spawnEnemy()
                self.spawnEnemy()
                levelNumber = 1
    ...
    func LivesPlayer(){
        ...
        if let live1 = childNode(withName: "live1"){
            ...
            sounds.play("HitSound", volume: gameData.volumeEffect)
            ...
            //print(live5)
            sounds.play("HitSound", speed: 0.5, volume: gameData.volumeEffect)
    ...
    func randomNormalMusic(){
        let musicFilenames = ["gameMainLow.wav", "InfinLow.wav", "Normalmusic.wav"]
    ...
    func handleCollisionPlayerFireWithBossOne(_ fireNode: SKNode?, _ bossOneNode: SKNode?) {
        ...
        if bossOneLives <= 0 {
            ...
            sounds.play("HitSound", pitch: 100, volume: gameData.volumeEffect)
            enemyActive = true
            SpeedOpposite = 2.0 - numUnknow1
            numUnknow1 += 0.02
            updateBossFireTimer()
            BossOneFireTimer?.invalidate()
            bossHealthLabel.isHidden = true
    ...
    func handleCollisionPlayerFireWithBossOnefire(_ fireNode: SKNode?, _ bossOnefireNode: SKNode?) {
        ...
        case 2:
            ...
            BossOneFire_Type2_Live -= PlayerFire_Type
        ...
        case 4:
            ...
            fireNode?.removeFromParent()
            BossOneFire_Type4_Live -= PlayerFire_Type
            SpawnExplosionBullet(spawnPosition: bossOnefireNode?.position ??  CGPoint.zero)
    ...
    func showBullyMessage() {
        let randomNumber = Double.random(in: 0...100)
        let chanceThreshold = 30.0
        if randomNumber <= chanceThreshold {
            let messageLabel = SKLabelNode(text: "Hah! Loser!")
            messageLabel.fontName = "Chalkduster"
            messageLabel.fontSize = 30
            messageLabel.fontColor = SKColor.red
            messageLabel.zPosition = 6
            messageLabel.position = CGPoint(x: bossyOne.position.x + 100, y: bossyOne.position.y)
            self.addChild(messageLabel)

            let waitAction = SKAction.wait(forDuration: 2.0)
            let removeAction = SKAction.removeFromParent()
            messageLabel.run(SKAction.sequence([waitAction, removeAction]))
        }
    }
    ...
    @objc func bossOneFireFunc(){
        ...
        sounds.play("Rocketlauch", volume: gameData.volumeEffect)
    ...
    func UpdateScore(_  Num: Int){
        ...
        let EnemyProbability = 0.05
        
        let randomValue = Double.random(in: 0...1)
        if countmusic >= 2{
            countmusic = 0
        }
        let muscilsit = ["Evil-BossLow.wav", "Evil-bossLow2.wav"]
        if !BossOneActive && !BossTwoActive {
            if gameData.score >= (10 + spaceTime2) && randomValue < (bossTwoSpawnProbability + SpawnCount2) {
                spaceTime2 += Int.random(in: 15...35)
                spawnBossTwo()
                AudioManager.shared.playBackgroundMusic(filename: muscilsit[countmusic])
                countmusic += 1
                ...
                enemyActive = false
                spawnBossOne()
                spaceTime1 += Int.random(in: 10...30)
                if SpawnCount2 >= 0.4 {
                    SpawnCount2 = 0.4
                } else {
                    SpawnCount2 += 0.05
    ...
    func updateBossFireTimer(){
        ...
        case 2:
            if BossTwoActive {
                if SpeedOpposite <= 2 && SpeedOpposite > 0.5{
                    BossTwoFireTimer?.invalidate()
                    BossTwoFireTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(bossTwoFireFunc), userInfo: nil, repeats: true)
                }
            }
        case 3:
            if BossOneActive{
    ...
    func fireBullet() {
        ...
        sounds.play("LaserGun", volume: gameData.volumeEffect)
    ...
    func gameOverFunc(){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1){
            //self.gameData.showGameScene = false
            self.gameData.isScoreView = true
...
struct ContentView: View {
    ...
    //@ObservedObject var game_Data1: gameData
                        ...
                        ZStack{
                            ...
                            VStack{
                                Button(action: {
                                    ...
                                    game_Data.score = 0
                                    game_Data.showGameScene = false
                                    game_Data.isScoreView = false
                                    ...
                                    .font(.custom("Chalkduster", size: geometry.size.width/25))
                                    .padding()
                                ...
                                Button(action: {
                                    ...
                                    game_Data.showGameScene = false
                                    game_Data.isScoreView = false
                                    AudioManager.shared.playBackgroundMusic(filename: "gameMusic.wav")
                                    AudioManager.shared.adjustVolume(volume: game_Data.volume)
                                }, label: {
                                    HStack{
                                        Image(systemName: "house")
                                ...
                                if game_Data.messagePopUp {
                                    Text("The score will be reset when you Exit.")
                                        .foregroundColor(.white)
                                        .transition(.opacity)
                                        .animation(.easeInOut(duration: 0.5), value: game_Data.messagePopUp
                                        )
                                }
                                Button(action: {
                                    clearAllScore()
                                    score = loadScore()
                                    game_Data.messagePopUp = true
                                    DispatchQueue.main.asyncAfter(deadline: .now() + 6){
                                        withAnimation{
                                            game_Data.messagePopUp = false
                                        }
                                    }
                                }, label: {
                                    VStack{
                            ...
                            .position(CGPoint(x: geometry.size.width/2, y: geometry.size.height/1.12))
                        }
                    }
                }
            }
    ...
    private func togglePause(){
        if game_Data.isPaused{
            ...
            game_Data.messageNoti = false
        } else {
            game_Data.NumPause = 1
            game_Data.isShowingAlert = true
