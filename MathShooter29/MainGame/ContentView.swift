...
class Game_Scene: SKScene, SKPhysicsContactDelegate{
    ...
    init(gameData: gameData ,size: CGSize) {
        ...
        //gameArea = CGRect(origin: .zero, size: screenSize)
    ...
    func setUpOnView(){
        ...
        if gameData.countGamePlay == 0 {
            /*
            let soundsToPreload = ["LaserGun.wav", "Rocketlauch.wav","HitSound.wav"]
            SKAction.preload(soundsToPreload){
                print("already")
            }*/
    ...
    func spawnEnemy(){/*
        let randomXstart = random(min: 0, max: UIScreen.main.bounds.width/1.1)
        ...
        */
        let randomXstart = random(min: gameArea.minX, max: gameArea.minX + size.width * 0.1)
        let randomXEnd = random(min: gameArea.maxX - size.width * 0.25, max: gameArea.maxX)
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?){
        for touch in touches {
            ...
            let sceneBounds = self.size
            if player.position.x > sceneBounds.width / 2 - player.size.width / 2 {
                        player.position.x = sceneBounds.width / 2 - player.size.width / 2
                    }
                    if player.position.x < -sceneBounds.width / 2 + player.size.width / 2 {
                        player.position.x = -sceneBounds.width / 2 + player.size.width / 2
                    }
                    if player.position.y > sceneBounds.height / 2 - player.size.height / 2 {
                        player.position.y = sceneBounds.height / 2 - player.size.height / 2
                    }
                    if player.position.y < -sceneBounds.height / 2 + player.size.height / 2 {
                        player.position.y = -sceneBounds.height / 2 + player.size.height / 2
                    }
    ...
    func changeScene(){
        ...
        //sceneToMoveTo.size = CGSize(width: 750, height: 1335)
            ...
            ZStack{
                ZStack(alignment: .center){
                    if game_Data.showGameScene{
                        SpriteView(scene: Game_Scene(gameData: game_Data, size: CGSize(width: geometry.size.width, height: geometry.size.height)), debugOptions: [.showsFPS, .showsNodeCount])
