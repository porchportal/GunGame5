...
struct StartView: View {
    ...
    @State private var positionR: (x: CGFloat, y: CGFloat)?
            ...
            ZStack {
                ZStack(alignment: .center){
                    if horizontalSizeClass == .regular && verticalSizeClass == .regular {
                        ...
                        AnimatedBackground()
                            .position(positionR != nil ? CGPoint(x: positionR!.x, y: positionR!.y) : CGPoint(x: 0, y: 0))
                            .opacity(isVisible ? 1.0 : 0.0)
                            .onAppear{
                                isVisible = true
                                positionR = (x: CGFloat.random(in: 100...geometry.size.width-120),
                                             y: CGFloat.random(in: 100...geometry.size.height-120))
                            }
                    }
                    AnimatedBackground()
...
struct AnimatedBackground: View {
    ...
    @State private var isViewVisible1 = false
    var body: some View {
        ZStack {
